...
/// Define a type alias that exists both in stub files AND at runtime.
///
/// Unlike [`type_alias!`], which only generates stub file entries, this macro creates
/// a struct that can be registered in Python modules at runtime using
/// [`runtime::PyModuleTypeAliasExt::add_type_alias`].
///
/// This is useful when you want the type alias to be importable from Python code,
/// not just available for static type checking.
///
/// # Syntax
///
/// ```rust,ignore
/// // Union type (most common)
/// define_type_alias! {
///     pub struct MyAlias in "module.name"; TypeA | TypeB
/// }
///
/// // Union type with documentation attribute
/// define_type_alias! {
///     /// Documentation for the type alias
///     pub struct MyAlias in "module.name"; TypeA | TypeB
/// }
///
/// // With explicit doc string
/// define_type_alias! {
///     pub struct MyAlias in "module.name"; TypeA | TypeB, doc = "My documentation"
/// }
/// ```
///
/// # Example
...
///
/// // Define a runtime type alias for a union type using Rust types
/// define_type_alias! {
...
///     // Register the type alias - now importable from Python
///     m.add_type_alias::<NumberOrString>()?;
///     Ok(())
/// }
/// ```
///
/// After this, Python code can do:
///
/// ```python
/// from my_module import NumberOrString
///
/// def greet(value: NumberOrString) -> str:
...
///
/// The macro generates:
/// 1. A zero-sized struct with the given name
/// 2. A [`PyStubType`] implementation for stub file generation
/// 3. A [`runtime::PyTypeAlias`] implementation for runtime registration
/// 4. An `inventory::submit!` call for metadata collection
///
/// # Note
///
/// The struct generated by this macro is zero-sized and serves only as a type-level
/// marker. It cannot be instantiated or used as a value.
#[macro_export]
macro_rules! define_type_alias {
    // Pattern 1: Union with explicit doc parameter
    (
        $(#[$meta:meta])*
        $vis:vis struct $name:ident in $module:literal; $($base:ty)|+, doc = $doc:expr
    ) => {
        $(#[$meta])*
        $vis struct $name;
    ...
    // Pattern 2: Union without doc (default pattern)
    (
        $(#[$meta:meta])*
        $vis:vis struct $name:ident in $module:literal; $($base:ty)|+
    ) => {
        $crate::define_type_alias! {
            $(#[$meta])*
            $vis struct $name in $module; $($base)|+, doc = ""
        }
    };
}
