...
inventory::submit! {
    derive::gen_methods_from_python! {
        ...
        class ClassB:
            @property
            def classes_b_manual(self, other: collections.abc.Generator[pyo3_stub_gen.RustType["PyClassA"], None, None]) -> pyo3_stub_gen.RustType["Vec<PyClassA>"]:
                ...
...
impl FromPyObject<'_, '_> for ClassACallback {
    type Error = PyErr;

    fn extract(ob: Borrowed<'_, '_, PyAny>) -> std::result::Result<Self, Self::Error> {
        Ok(ClassACallback(ob.to_owned().unbind()))
    }
}
