...
In addition to generating `.pyi` stub files, `pyo3-stub-gen` can generate **Sphinx-compatible API reference documentation** with full intersphinx support. This addresses limitations in existing tools (sphinx-autodoc2, mkdocstrings) that cannot properly handle:

- `@overload`-only functions (all signatures are preserved)
- Type alias definitions with docstrings
- Type alias preservation (not expanded)
- Proper re-export handling
- Accurate link resolution for re-exported items

### Quick Start

**1. Enable documentation generation in `pyproject.toml`:**
...
This creates:
- `docs/api/api_reference.json` - The documentation IR
- `docs/api/pyo3_stub_gen_ext.py` - The Sphinx extension
...
# Add the API docs directory to Python path
sys.path.insert(0, str(Path(__file__).parent / 'api'))

extensions = [
    'pyo3_stub_gen_ext',           # Our extension
...
**4. Create documentation page (`docs/api.rst`):**
...
The generated documentation fully supports Sphinx's intersphinx cross-referencing system. You can reference your API from anywhere in your documentation:

**Reference functions:**
```rst
See {py:func}`mymodule.add` for adding numbers.
See {py:func}`~mymodule.calculate_complex` for details.
```

The `~` prefix shows only the last component (e.g., `calculate_complex` instead of `mymodule.calculate_complex`).

**Reference classes and methods:**
```rst
The {py:class}`mymodule.MyClass` provides...
Use {py:meth}`mymodule.MyClass.my_method` to...
Use {py:attr}`mymodule.MyClass.my_attr` to access...
```

**Reference type aliases and constants:**
```rst
See {py:data}`mymodule.MyTypeAlias` for the type definition.
The {py:data}`mymodule.CONSTANT` value is...
```

**In MyST Markdown:**
```markdown
See {py:func}`mymodule.add` for details.
The {py:class}`~mymodule.DataContainer` class...
```

### Cross-Project References

Other Sphinx projects can reference your API via intersphinx:

**In their `conf.py`:**
```python
intersphinx_mapping = {
    'your_project': ('https://your-docs-url.com/', None),
}
```

**In their documentation:**
```rst
This function uses {py:func}`your_project.helper` internally.
```
...
All `@overload` signatures are preserved and displayed:

```python
def as_tuple(xs: Sequence[int], tuple_out: Literal[False]) -> list[int]: ...
def as_tuple(xs: Sequence[int], tuple_out: Literal[True]) -> tuple[int, ...]: ...
```

**✅ Type Aliases**

Type aliases are preserved with their definitions and docstrings:

```python
CallbackType: TypeAlias = collections.abc.Callable[[str], None]
"""A callback function that takes a string and returns None."""
```

**✅ Complete Type Information**

All parameters and return types include full type annotations.

**✅ Python Domain Registration**

All documented items (functions, classes, methods, type aliases, variables) are properly registered with Sphinx's Python domain, enabling:
- Automatic inventory generation (`objects.inv`)
- Full intersphinx support
- Proper cross-referencing throughout your documentation

### Multiple Modules

For mixed layouts with submodules, document each separately:
...
```toml
[tool.pyo3-stub-gen.doc-gen]
output-dir = "docs/api"           # Where to generate documentation files
json-output = "api_reference.json" # Name of the JSON IR file
```

Both options have defaults and are optional. If `[tool.pyo3-stub-gen.doc-gen]` section exists, documentation generation is enabled.

### Troubleshooting

**Extension not found:**
- Ensure `sys.path.insert(0, str(Path(__file__).parent / 'api'))` is in your `conf.py`
- Check that `pyo3_stub_gen_ext.py` exists in `docs/api/`

**Module not found in JSON:**
- Verify `api_reference.json` exists in the output directory
- Check the module name matches what's in the JSON
- Re-run `cargo run --bin stub_gen`

**Cross-references not working:**
- Ensure `sphinx.ext.intersphinx` is in the `extensions` list
- Build with `-E` flag to clear cache: `sphinx-build -E -b html . _build/html`
- Check that `objects.inv` is generated in your `_build/html/` directory
