...
| [examples/mixed] | Example for [Mixed Rust/Python maturin project](https://www.maturin.rs/project_layout#mixed-rustpython-project) with user-written `__init__.py` |
...
[examples/generate_init_py]: ./examples/generate_init_py/
...
- **Stub files**: Type checkers see re-exported items in the parent module
- **Runtime**: Python code can import from the parent module
- **Documentation**: API docs show items under the parent module

See [examples/generate_init_py](./examples/generate_init_py/) for a complete working example.
...
- [examples/generate_init_py/docs/](./examples/generate_init_py/docs/) — mixed layout with `reexport_module_members!`

### Limitation: Mixed Layout with User-written `__init__.py`

Docgen works well with **pure** layout and **mixed layout using `reexport_module_members!`** (e.g., `generate_init_py` example). However, it has limitations with mixed layout projects where users write their own `__init__.py` with custom re-exports.

In such cases, docgen cannot automatically detect which items are re-exported from user-written Python code. The generated documentation will only include items from PyO3 modules, not the re-export structure defined in `__init__.py`. For projects requiring custom Python integration, consider documenting the public API manually or using `reexport_module_members!` to declare re-exports in Rust.
